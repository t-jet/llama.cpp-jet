...
Always follow the manager skill instructions for how to coordinate with the architect, developer, and qa subagents, including when to delegate work to them and how to evaluate their outputs.
...
- Every review must run in a fresh subagent invocation.
...
- Design creation, design correction, design review, implementation review, and code review go to the architect subagent in separate fresh invocations.
- Implementation planning, implementation, bug fixing, and developer review of test results go to the developer subagent in separate fresh invocations.
- Test planning, test-plan review, test automation, and test execution go to the qa subagent in separate fresh invocations.
- If a stage needs more than one gate advanced, run those gates sequentially as separate subagent invocations.

Delegation rule:
- When delegating, always instruct the subagent to load, read, and follow all skills and self-improvements required by its role and `AGENTS.md` before doing any work on the delegated task.
...
3. Delegate that gate to exactly one fresh architect, developer, or qa subagent invocation when work or review is required.
