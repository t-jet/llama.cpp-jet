...
The referenced discussion on PR #13194 matters directly to this plan. The key point from the linked comment is that the SWA implementation was only expected to evaluate correctly as long as the user was not relying on context shift, cache reuse, or branching from old states. In other words, some backends can still run inference correctly while not supporting the full set of reuse/editing operations that the server's prompt-cache and checkpoint logic would ideally like to use.

The current tree already reflects this distinction:

- common/common.h and common/common.cpp classify contexts as PART, FULL, RS, or NO via common_context_can_seq_rm()
- tools/server/server-context.cpp creates checkpoints when the backend does not support partial rollback, only supports bounded rollback, or uses SWA
- tools/server/server-context.cpp can still fall back to forcing full prompt re-processing when there is no valid cache data to restore, with a log that explicitly points at SWA or hybrid/recurrent memory

Implication for this plan:

- compression is a storage optimization only
- compression does not add missing cache-reuse semantics
- compression does not make context shift, cache reuse, or branching from old states suddenly valid on backends where they are not supported
- compression is likely most useful for configurations that deliberately preserve reusable state, for example swa_full-like behavior or other backends where the server can actually restore meaningful old state

This is the main correction that should be carried through the requirements and implementation sections below.
...
10. Compression must be capability-aware: it may only optimize states that the current backend and server path already consider storable/restorable; it must not be treated as a way to enable unsupported context shift, cache reuse, or branching.
11. Existing request/response APIs, including `timings.cache_n` and `timings.prompt_n`, must remain unchanged.
12. Existing slot/session file formats and `/slots?action=save|restore` semantics must remain unchanged.
13. The implementation must fail soft: compression failure must not break inference or prompt reuse; it should log and fall back to raw storage or skip storing the entry.
...
- The documentation for these options should explicitly say that compression does not fix SWA or hybrid/recurrent reuse limitations by itself.
- For SWA-style backends, users who need advanced cache reuse may still need a full-cache mode such as `--swa-full`, or may need to avoid context shift for the relevant workload.
