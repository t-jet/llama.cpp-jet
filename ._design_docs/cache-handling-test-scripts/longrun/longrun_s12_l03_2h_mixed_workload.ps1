...
# Stage 12 long-run: S12-L03 legacy control 2 hour.
# Sampler interval 60 s. Verifies legacy path stays stable; no hybrid
# metric rows active; no cold-store side effects.
# Evidence dir: ._design_docs/.test_reports/longrun-s12-l03-<timestamp>/
...
$flags = @('--cache-mode','legacy','--parallel','1','--cache-ram','100',
           '--metrics','--ctx-size','512','--temp','0','--seed',"$Seed")
$flags = Merge-MtpJinjaFlag -Flags $flags -JinjaPath $jinjaPath

Write-Host "S12-L03 2h legacy control; stub=$stubData"

if ($DurationMin -gt 0) {
    Write-Host "DRY-RUN: would run $DurationMin min legacy with $SamplerIntervalS s sampler; QA verifies no hybrid metric rows"
} else {
    Write-Host "DRY-RUN: would run $DurationHours h legacy with $SamplerIntervalS s sampler; QA verifies no hybrid metric rows"
}
if ($DryRun) { exit 0 }
...
if ($stubData) {
    ...
    Write-LongrunEvidence -OutDir $OutDir -ScenarioId 'S12-L03' -Variant 'legacy-2h' `
...
while ((Get-Date) -lt $endTime) {
    $body = '{"prompt":"S12-L03 legacy control probe","n_predict":2,"temperature":0,"seed":42}'
    try {
...
Write-LongrunEvidence -OutDir $OutDir -ScenarioId 'S12-L03' -Variant 'legacy-2h' `
    ...
    -Verdict PENDING -Notes "Live run; QA verifies legacy path is stable with no hybrid rows"
