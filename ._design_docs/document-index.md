...
| [cache-handling-phase8-design.md](cache-handling-phase8-design.md) | Stage 8 design entry for metadata-only nodes and re-materialization, with links to split part files. Status is ready for independent design review; no implementation gate is approved. | Use this to review the proposed Stage 8 scope, lifecycle rules, metadata-only retention, re-materialization validation, mismatch-parent selection, equivalent-branch deduplication, cold cleanup safety, branch-metadata pruning, observability, testability, requirement traceability, exclusions, risks, and review readiness. |
...
| [cache-handling-test-plan.md](cache-handling-test-plan.md) | Cache implementation test plan entry point, with links to detailed part files. Covers Stage 4 byte-budget and protected-root behavior, Stage 5 descriptor and draft-pairing behavior, Stage 6 cold layer integration, Stage 7 branch graph foundation coverage, metrics, reusable evidence rules, and local Qwen3 target/draft model-suite guidance. | Use this to find manual and scripted test cases for cache behavior and QA execution reporting. |
