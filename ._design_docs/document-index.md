...
| [cache-handling-test-plan.md](cache-handling-test-plan.md) | Cache implementation test plan entry point, with links to detailed part files. Covers closed Stage 5 descriptor separation, draft context mode compatibility, target/draft transactional restore verification, Stage 4 regression coverage, Stage 6 cold layer integration, metrics, reusable evidence rules, and local Qwen3 target/draft model-suite guidance. | Use this to find manual and scripted test cases for cache behavior and QA execution reporting. |
