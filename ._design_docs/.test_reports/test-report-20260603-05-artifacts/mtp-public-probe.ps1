# MTP public probe for T121 - Stage 10 bug-fix loop (2026-06-04)
# Drives llama-server with the Qwen3.5-4B MTP fixture using the corrected
# server configuration: --cache-mode hybrid --cache-ram 512 --spec-type draft-mtp
# Exercises /completion requests with longer prompts and deterministic
# boundaries, then compares cache_checkpoint_* rows in /metrics before/after.
...
param(
    ...
    [int]$Port = 8185,
    [int]$WaitSeconds = 240,
    [string]$OutDir = 'D:\source\llama.cpp-jet\._design_docs\.test_reports\test-report-20260603-05-artifacts'
)
...
Write-Host "Starting llama-server on port $Port with MTP fixture and hybrid cache..."
...
# Attempt 1: longer completion with deterministic boundary
$prompt1 = ("In a quiet coastal town, a lighthouse keeper named Maren kept a journal of every ship that passed. " * 4).Trim()
$payload1 = @{
    prompt      = $prompt1
    n_predict   = 64
    temperature = 0
...
# Attempt 2: longer completion with MTP-flavored params and a different deterministic boundary
$prompt2 = ("The capital of France is Paris. The capital of Germany is Berlin. " * 8).Trim()
$payload2 = @{
    prompt      = $prompt2
    n_predict   = 96
    temperature = 0
