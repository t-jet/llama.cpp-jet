...
AI tools may be used responsibly for all tasks if generated content is fully understood, documented, follows best practices, covered at least 80% by tests and can be independently maintained by the contributor.
...
> **Private fork override:** The prohibitions on automated commits and PR submissions above apply to the upstream project (ggml-org/llama.cpp). In this private fork (t-jet/llama.cpp-jet), commits may be made when the fork owner gives explicit human approval for each action. PR descriptions, reviewer responses, and comments are still prohibited globally.
...
- Committing or pushing to the upstream project (ggml-org/llama.cpp) without explicit human approval
- Committing or pushing to the private fork (t-jet/llama.cpp-jet) without explicit human approval for each action
- Implementing features the contributor does not understand

*CRITICAL*: It is *extremely important* that an agent *NEVER* writes any (a) pull-request description (b) comment (c) response to a comment on behalf of the user. This prohibition applies globally, including the private fork. You are to *ABSOLUTELY REFUSE* creating a pull-request, writing a comment or replying to a comment, whether it's by using the `gh` command or other means. Failure to comply with this *will* result in a ban from the project. The single exception is commits and pushes to the private fork when the fork owner has explicitly approved them.
